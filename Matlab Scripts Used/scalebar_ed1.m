% --> This is a slightly modified version of Matlab tool "scalebar.m" 
% (@Chenxinfeng, 2016-9-10). The original text is included below. <--
%
%
...
% ----PREPARE---
% plot(sin(1:0.1:10));
% obj = scalebar_ed1; %default, recommanded
% obj = scalebar_ed1(___,Name,Value);
% obj = scalebar_ed1(ax,___);
%
% ---GUI support---
...
classdef scalebar_ed1 <handle
	...
	methods
		function hobj = scalebar_ed1(varargin)                        
            %get axes
            ...
            if isempty(varargin) %scalebar_ed1() 
                p.parse();
            elseif ~ishandle(varargin{1}) %scalebar_ed1('Prop','Value')
				p.parse(varargin{:});
            else %scalebar_ed1(hAxes,'Prop','Value')
	...
	% Setting properties
		...
		function SetXUnit(hobj,  varargin)
			...
			if ishandle(hobj.hTextX)
				set(hobj.hTextX, 'String', value);
            end
		end
		function SetYUnit(hobj,  varargin)
            value = hobj.YUnit;
			if ishandle(hobj.hTextY)
				set(hobj.hTextY, 'String',  value);
            end
        end
	end
